...
| createdDateTime | DateTimeOffset | The time when the delegated permission grant was created. Read-only. |
| expiryTime | DateTimeOffset | Currently, the expiry time value is ignored, but a value is required when creating an **oAuth2PermissionGrant**. Required. |
| id | String | Unique identifier for the **oAuth2PermissionGrant**. Read-only.|
| lastModifiedDateTime | DateTimeOffset | The time when the delegated permission grant was last updated. Read-only. |
