...
### Example 3: Convert a single-tenant application to limited multi-tenant

#### Request
The following request shows an example updating an application's **signInAudience** from single-tenant (`AzureADMyOrg`) to multi-tenant (`AzureADMultipleOrgs`) and at the same time setting **signInAudienceRestrictions** so that the app can only be used in the given list of allowed tenants.
...
### Example 4: Convert a restricted multi-tenant application to be single-tenant

#### Request

The following request shows an example updating an application's **signInAudience** to single-tenant (`AzureADMyOrg`) and at the same removing  **signInAudienceRestrictions**. It's recommended to do this at the same time to avoid any time where the application is multi-tenant and *not* limited, which may be required by the the organization's policies.
